...
class PositionEmbeddingCamRay(nn.Module):
    ...
    def __init__(self, num_pos_feats=64, temperature=10000, normalize=False, scale=None):
        ...
        self.LID = True
        self.range_num = 81
        self.range_start = 1e-3
        self.num_pos_feats = num_pos_feats
        self.normalize = normalize
        self.range_dim_keep = 24

        self.position_dim = 3 * self.range_num      # D*3 3:(x, y, z)
        self.range_position_dim = 3 * self.range_dim_keep  # K*3 3:(x, y, z)
        self.embed_dims = 256
        ...
        self.range_position_encoder = nn.Sequential(
                nn.Conv2d(self.range_position_dim, self.embed_dims, kernel_size=1, stride=1, padding=0),
                nn.ReLU(),
                nn.Conv2d(self.embed_dims, self.embed_dims, kernel_size=1, stride=1, padding=0),
            )
        self.position_sine = PositionEmbeddingSine(num_pos_feats, normalize=True)
        
    def forward(self, tensor_list: NestedTensor, calibs=None, range_map=None):
        """
        Args:
        ...
        if self.LID:
            # (D, )
            index = torch.arange(start=0, end=self.range_num, step=1, device=img_feats[0].device).float()
            index_1 = index + 1
            bin_size = (self.position_range[3] - self.range_start) / (self.range_num * (1 + self.range_num))
            coords_d = self.range_start + bin_size * index * index_1
        else:
            index = torch.arange(start=0, end=self.range_num, step=1, device=img_feats[0].device).float()
            bin_size = (self.position_range[3] - self.range_start) / self.range_num
            coords_d = self.range_start + bin_size * index
        ...
        if range_map is not None:
            range_map = F.interpolate(range_map, size=img_feats.shape[2:], mode='bilinear', align_corners=True)
            range_score = F.softmax(range_map, dim=1)
            _, keep_indices = torch.topk(range_score, k=self.range_dim_keep, dim=1)
            # (B, W, H, K)
            ...
            if calibs is not None:
                ...
                rays_position_embeding = self.range_position_encoder(rays)      # (B, embed_dims, H, W)
            ...
            coords3d = coords.permute(0, 3, 4, 2, 1).contiguous().view(B, -1, H, W)
            # 3D position embedding(PE)
            coords_position_embeding = self.range_position_encoder(coords3d)      # (B, embed_dims, H, W)
