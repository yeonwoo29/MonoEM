...
class Det3DTransformer(nn.Module):
    def __init__(
        ...
        decoder_layer = rangeAwareDecoderLayer(
            d_model, dim_feedforward, dropout, activation, num_feature_levels, nhead, dec_n_points, group_num=group_num)
        self.decoder = rangeAwareDecoder(decoder_layer, num_decoder_layers, return_intermediate_dec)
    ...
    def forward(self, intermediate_output, query_embeds, range_pos_embed):
        ...
        range_pos_embed = range_pos_embed.flatten(2).permute(2, 0, 1)
        mask_range = None
        query_embeds = None
...
class rangeAwareDecoderLayer(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024,
        ...
        # range cross attention
        self.cross_attn_range = nn.MultiheadAttention(d_model, n_heads, dropout=dropout)
        self.dropout_range = nn.Dropout(dropout)
        self.norm_range = nn.LayerNorm(d_model)
    ...
    def forward(self,
        ...
        # range cross attention
        tgt2 = self.cross_attn_range(tgt.transpose(0, 1),
                                     range_pos_embed,
                                     range_pos_embed,
                                     key_padding_mask=mask_range)[0].transpose(0, 1)
       
        tgt = tgt + self.dropout_range(tgt2)
        tgt = self.norm_range(tgt)
...
class rangeAwareDecoder(nn.Module):
    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
                query_pos=None, src_padding_mask=None, range_pos_embed=None, mask_range=None, bs=None):
        output = tgt
