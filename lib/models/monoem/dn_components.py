...
def prepare_for_dn(dn_args, tgt_weight, embedweight, batch_size, training, num_queries, num_classes, hidden_dim, label_enc):
    ...
    if training:
        ...
        ## for 3d
        ranges = torch.cat([t['range'] for t in targets])
        ...
        #for 3d
        known_ranges = ranges.repeat(scalar, 1)
        ...
        mask_dict = {
            ...
            'known_lbs_bboxes': (known_labels, known_bboxs, known_size_3ds, known_ranges, known_target_heading_cls, known_target_heading_res),
            'know_idx': know_idx,
            'pad_size': pad_size
        }
...
def dn_post_process(outputs_class, outputs_coord, outputs_3d_dim,outputs_range,outputs_angle,mask_dict):
    """
    ...
    if mask_dict and mask_dict['pad_size'] > 0:
        ...
        output_known_range = outputs_range[:, :, :mask_dict['pad_size'], :]
        output_known_angle = outputs_angle[:, :, :mask_dict['pad_size'], :]

        outputs_class = outputs_class[:, :, mask_dict['pad_size']:, :]
        outputs_coord = outputs_coord[:, :, mask_dict['pad_size']:, :]
        outputs_3d_dim = outputs_3d_dim[:, :, mask_dict['pad_size']:, :]
        outputs_range = outputs_range[:, :, mask_dict['pad_size']:, :]
        outputs_angle = outputs_angle[:, :, mask_dict['pad_size']:, :]
        mask_dict['output_known_lbs_bboxes']=(output_known_class,output_known_coord,output_known_3d_dim,output_known_range,output_known_angle)
    return outputs_class, outputs_coord, outputs_3d_dim, outputs_range, outputs_angle
...
def prepare_for_loss(mask_dict):
    ...
    output_known_class, output_known_coord, output_known_3d_dim,output_known_range,output_known_angle = mask_dict['output_known_lbs_bboxes']
    known_labels, known_bboxs, known_size_3ds, known_ranges, known_target_heading_cls, known_target_heading_res = mask_dict['known_lbs_bboxes']
    ...
    if len(output_known_class) > 0:
        ...
        output_known_range = output_known_range.permute(1, 2, 0, 3)[(bid, map_known_indice)].permute(1, 0, 2)
        output_known_angle = output_known_angle.permute(1, 2, 0, 3)[(bid, map_known_indice)].permute(1, 0, 2)
    num_tgt = known_indice.numel()
    return known_labels, known_bboxs, known_size_3ds, known_ranges, known_target_heading_cls, known_target_heading_res, output_known_class, output_known_coord, output_known_3d_dim,output_known_range,output_known_angle, num_tgt
...
def tgt_loss_ranges(src_range, tgt_range, num_tgt):  

    range_input, range_log_variance = src_range[:, 0], src_range[:, 1] 
    range_loss = 1.4142 * torch.exp(-range_log_variance) * torch.abs(range_input - tgt_range) + range_log_variance  
    losses = {}
    losses['tgt_loss_range'] = range_loss.sum() / num_tgt 
    return losses  
...
def compute_dn_loss(mask_dict, training, aux_num, focal_alpha):
    ...
    if training and 'output_known_lbs_bboxes' in mask_dict:
        known_labels, known_bboxs, known_size_3ds, known_ranges, known_target_heading_cls, known_target_heading_res,\
        output_known_class, output_known_coord, output_known_3d_dim,output_known_range,output_known_angle, num_tgt = prepare_for_loss(mask_dict)
        ...
        # losses.update(tgt_loss_ranges(output_known_range[-1], known_ranges, num_tgt))
        ...
        # losses['tgt_loss_range'] = torch.as_tensor(0.).to('cuda')
