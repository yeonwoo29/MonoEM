...
def decode_detections(dets, info, calibs, cls_mean_size, threshold):
    ...
    for i in range(dets.shape[0]):  # batch
        ...
        for j in range(dets.shape[1]):  # max_dets
            ...
            locations = calibs[i].img_to_rect(x3d, y3d, range).reshape(-1)
...
def extract_dets_from_outputs(outputs, K=50, topk=50):
    ...
    range = outputs['pred_range'][:, :, 0: 1]
    sigma = outputs['pred_range'][:, :, 1: 2]
    ...
    range = torch.gather(range, 1, topk_boxes)
    ...
    detections = torch.cat([labels, scores, xs2d, ys2d, size_2d, range, heading, size_3d, xs3d, ys3d, sigma], dim=2)
