...
class Calibration(object):
    ...
    def rect_to_img(self, pts_rect):
        ...
        pts_rect_range = pts_2d_hom[:, 2] - self.P2.T[3, 2]  # range in rect camera coord
        return pts_img, pts_rect_range
    ...
    def lidar_to_img(self, pts_lidar):
        ...
        pts_img, pts_range = self.rect_to_img(pts_rect)
        return pts_img, pts_range

    def img_to_rect(self, u, v, range_rect):
        """
        :param u: (N)
        :param v: (N)
        :param range_rect: (N)
        :return:
        """
        x = ((u - self.cu) * range_rect) / self.fu + self.tx
        y = ((v - self.cv) * range_rect) / self.fv + self.ty
        pts_rect = np.concatenate((x.reshape(-1, 1), y.reshape(-1, 1), range_rect.reshape(-1, 1)), axis=1)
        return pts_rect

    def rangemap_to_rect(self, range_map):
        """
        :param range_map: (H, W), range_map
        :return:
        """
        x_range = np.arange(0, range_map.shape[1])
        y_range = np.arange(0, range_map.shape[0])
        x_idxs, y_idxs = np.meshgrid(x_range, y_range)
        x_idxs, y_idxs = x_idxs.reshape(-1), y_idxs.reshape(-1)
        range = range_map[y_idxs, x_idxs]
        pts_rect = self.img_to_rect(x_idxs, y_idxs, range)
...
if __name__ == '__main__':
    ...
    for object in objects:
        ...
        center_3d_projected, range = calib.rect_to_img(center_3d)
        box2d = object.box2d
