...
class GradientLoss(nn.Module):
    ...
    def forward(self, gen_image, real_image):
        gen_dx, gen_dy = self.gradient(gen_image)
        real_dx, real_dy = self.gradient(real_image)
        grad_diff_x = torch.abs(gen_dx - real_dx) * 2
        grad_diff_y = torch.abs(gen_dy - real_dy)
